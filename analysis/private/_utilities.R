...
# Zip study's results i.e. all files and folders under the `results` folder except rds files that might contain patient level data
...
# Job Log: Type of analysis string i.e. baseline or post-index to determine log details and filename(s)
jobTypeOfAnalysis <- function(type) {
  
  typeOfAnalysis <- as.data.frame(x = type) %>%
...
# Job Log: Start of baseline or post-index characterization. Shows target cohorts and time windows of execution
jobStartCharacterization <- function(typeOfAnalysis, cohortKey, timeA, timeB) {
  
  cli::cat_boxx(crayon::magenta(paste0("Building ", typeOfAnalysis$fullName, " covariates")))
  cli::cat_bullet(paste0("Using ", typeOfAnalysis$fullName,  " window: [", crayon::green(timeA), ", ", crayon::green(timeB), "]"),
...
# Job Log: Start of baseline or post-index characterization. Shows target cohorts and time windows of execution
jobFinishedExecution <- function(tok, tdif) {
